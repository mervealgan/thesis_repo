...
print(f"🧠 PCA retained {explained:.2%} of total variance")

X_embeddings_pca_df = pd.DataFrame(X_embeddings_pca, columns=[f"pca_{i+1}" for i in range(X_embeddings_pca.shape[1])])

X_coref = pd.read_csv("coref_features_clean.csv")

X = pd.concat([X_embeddings_pca_df, X_readabilityfeatures, X_coref], axis=1)
...
# joblib.dump(pca, "pca_model_03.pkl")
...
for name, model in models.items():
    ...
    joblib.dump(model, f"{name.replace(' ', '_').lower()}_pca_model.pkl")
...
# For the best performing model, analyze feature importance
