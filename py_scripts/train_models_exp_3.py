...
for name, model in models.items():
    ...
    # Saving model
    joblib.dump(model, f"{name.replace(' ', '_').lower()}_exp3_model.pkl")
    ...
    predictions_df.to_csv(f"{name.replace(' ', '_').lower()}_exp3_predictions.csv", index=False)
...
# Save results
pd.DataFrame(results).T.to_csv("exp3_results.csv")