...
# Combining all input
X = pd.concat([X_embeddings, X_read, X_coref], axis=1)
...
for name, model in models.items():
    ...
    # Saving model
    joblib.dump(model, f"{name.replace(' ', '_').lower()}_exp2_model.pkl")
    ...
    predictions_df.to_csv(f"{name.replace(' ', '_').lower()}_exp2_predictions.csv", index=False)
...
# Save results
pd.DataFrame(results).T.to_csv("exp2_results.csv")