...
X_embeddings = pd.read_csv("embedding_diff_attn_pool.csv")
...
for name, model in models.items():
    ...
    # Saving model
    joblib.dump(model, f"{name.replace(' ', '_').lower()}_exp4_model.pkl")
    ...
    predictions_df.to_csv(f"{name.replace(' ', '_').lower()}_exp4_predictions.csv", index=False)
...
# Save results
pd.DataFrame(results).T.to_csv("exp4_results.csv")